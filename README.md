...
InfoPeek is a cross-browser (Chrome / Edge / Firefox) developer extension with two tools:
...
**Privacy-first by design:** no network requests, no analytics, dynamic per-site permissions, everything processed locally. See [PRIVACY.md](./PRIVACY.md).
...
- **Chrome / Edge**: unzip `infopeek-1.0.0-chrome.zip`, open `chrome://extensions`, enable Developer mode, "Load unpacked".
- **Firefox**: open `about:debugging` → This Firefox → Load Temporary Add-on → pick the `manifest.json` inside `infopeek-1.0.0-firefox.zip`.
...
Your data never leaves your browser. No uploads, no tracking, no network requests (except optional remote-media rendering, off by default). Full policy: [PRIVACY.md](./PRIVACY.md).
