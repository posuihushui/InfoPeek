...
- **Chrome / Edge**: unzip `infopeek-1.1.0-chrome.zip`, open `chrome://extensions`, enable Developer mode, "Load unpacked".
- **Firefox**: open `about:debugging` → This Firefox → Load Temporary Add-on → pick the `manifest.json` inside `infopeek-1.1.0-firefox.zip`.
